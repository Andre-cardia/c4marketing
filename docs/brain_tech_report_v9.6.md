...
## Bloco 8 — Novo Ciclo v9.2: Memória Viva Corporativa + Confiabilidade Operacional (Segundo Cérebro)

> Data de fechamento deste ciclo: **1 de março de 2026**.

### 8.1 Contexto do ciclo v9.2

Após validação operacional do Segundo Cérebro em produção e ambiente local, foi identificado um desvio crítico em relação ao objetivo original da memória cognitiva corporativa:

- a base de memória explícita não estava sendo recuperada com consistência;
- o fluxo de "acabei de pedir para salvar" retornava fatos antigos;
- havia risco de percepção de memória "não viva" por falta de determinismo na recuperação imediata.

O ciclo v9.2 foi executado para corrigir essa lacuna com foco em:

1. **determinismo na recuperação de memória explícita**;
2. **governança de acesso por perfil (somente gestor)**;
3. **telemetria e canário confiáveis para regressão**;
4. **higiene arquitetural para operações de cron sem segredos hardcoded**.

---

### 8.2 Diagnóstico técnico consolidado

Principais causas identificadas durante o incidente de memória:

1. O fluxo de recall imediato podia cair em fallback semântico amplo e recuperar marcadores históricos (ex.: `DBG_MARKER_*`) em vez do fato recém-salvo.
2. O histórico de sessão usado como fonte de verdade não era suficiente, isoladamente, para cobrir todos os caminhos de recuperação explícita.
3. O monitoramento de regressão não detalhava a origem do recall em caso de falha (faltava visibilidade de `source/scope`).
4. Existia fragilidade de segurança funcional: o produto exige que o Segundo Cérebro seja exclusivo para perfil **gestor**.

---

### 8.3 Implementações no backend `chat-brain` (Segundo Cérebro)

Arquivo principal impactado:

- `supabase/functions/chat-brain/index.ts`

#### 8.3.1 Guardrail de acesso por perfil (somente gestor)

Foi implementado bloqueio explícito no backend para impedir uso do Segundo Cérebro por perfis não gestores.

Comportamento:

- normalização de role (`lowercase` + remoção de acentos);
- se `role != gestor`, resposta `403` com mensagem de acesso negado;
- metadados de resposta indicando role recebida e role exigida.

Impacto:

- garante aderência à regra de negócio do produto;
- evita bypass via chamadas diretas ao endpoint.

#### 8.3.2 Reforço da memória explícita (save/recall)

Foi consolidada a camada de memória explícita com:

- persistência de fatos explícitos com metadados estruturados (`source=explicit_user_memory`, `tenant_id`, `session_id`, `saved_at`, etc.);
- parsing dedicado de fatos explícitos salvos;
- detecção de intenção de recall explícito (ex.: "qual informação eu acabei de pedir para salvar?").

#### 8.3.3 Recuperação determinística com priorização de fontes

A recuperação da última memória explícita passou a seguir ordem de prioridade controlada:

1. histórico da sessão atual (resposta de confirmação de memória);
2. último comando de salvar detectado na sessão atual;
3. histórico cross-session do usuário (quando aplicável);
4. armazenamento de fatos explícitos (RPC dedicada);
5. fallback cognitivo semântico como último recurso.

Esse encadeamento reduziu respostas inconsistentes e diminuiu dependência de busca semântica global para perguntas de curto prazo.

#### 8.3.4 Fallback cognitivo com janela temporal recente

No caminho de fallback cognitivo, foi adicionada priorização temporal:

- primeira tentativa com janela curta (recentes horas) para capturar o "acabei de salvar";
- somente em caso de vazio, tentativa em histórico completo.

Resultado:

- eliminação do comportamento que puxava memória antiga quando o usuário pedia recall imediato.

#### 8.3.5 Metadados de rastreabilidade de memória

A resposta passou a carregar metadados de depuração operacional:
...
Isso melhora auditoria, investigação de regressão e suporte técnico.

#### 8.3.6 Robustez em perguntas compostas (multi-RPC)

Foi reforçado o normalizador global de decisão `db_query` para reduzir respostas parciais em perguntas compostas (ex.: usuários + projetos).

Complementos implementados:

- enriquecimento e deduplicação de chamadas;
- reforço determinístico de cobertura (`enforceCompositeCoverage`);
- normalização também para decisões vindas de caminhos heurísticos/fallback.

---

### 8.4 Migrações SQL criadas no ciclo v9.2

#### 8.4.1 Gestão segura de cron sem hardcode de URL/chave
...
- `public.schedule_brain_sync_job(p_url, p_service_role_key, p_job_name, p_schedule)`
- `public.unschedule_brain_sync_job(p_job_name)`

Objetivo:

- remover segredo hardcoded de migration;
- parametrizar por ambiente;
- viabilizar rotação de chave e governança segura.

#### 8.4.2 RPC determinística para fatos explícitos recentes
...
- `public.get_recent_explicit_user_facts(p_user_id, p_session_id, p_limit)`

Objetivo:

- fornecer caminho determinístico de recuperação por usuário/sessão;
- deduplicar fatos e priorizar escopo de sessão;
- reduzir instabilidade de recall por similaridade vetorial.

---

### 8.5 Melhorias no canário operacional
...
1. JWT sintético corrigido para formato válido em 3 segmentos.
2. Role do usuário de teste parametrizada (`BRAIN_TEST_USER_ROLE`, default `gestor`).
3. Captura de erro de rede com causa detalhada.
4. Em falha de recall, impressão de `t4.answer` e `t4.meta` para análise forense rápida.

---

### 8.6 Execução, deploy e validação

Deploys realizados:

- múltiplos deploys incrementais da função `chat-brain` no projeto Supabase `xffdrdoaysxfkpebhywl`.

Evolução dos resultados do canário:

1. estado inicial com falhas de autenticação/JWT;
2. evolução para **4/5** com zero falhas críticas (falhando apenas recall imediato);
3. após ajustes de priorização/fallback, resultado final **5/5**.

Resultado final consolidado:

- **Saúde + Telemetria + Auditoria: PASS**
- **Pergunta composta (multi-RPC): PASS**
- **Salvar memória explícita: PASS**
- **Recuperação imediata da memória: PASS**
- **Hierarquia normativa: PASS**
...
### 8.8 Arquivos impactados no ciclo v9.2 (Segundo Cérebro)
...
### 8.9 Checklist de aceite v9.2

- [x] Segundo Cérebro restrito a usuários com perfil gestor.
- [x] Memória explícita salva com metadados consistentes.
- [x] Recall imediato estabilizado para "acabei de pedir para salvar".
- [x] Fallback cognitivo com priorização temporal recente.
...
## Bloco 9 — Novo Ciclo v9.5: Hardening Operacional Pós-Go-Live (03 de março de 2026)

> Data de fechamento deste ciclo: **3 de março de 2026**.

### 9.1 Objetivo do ciclo v9.5

Após o fechamento técnico da v9.2, o ciclo v9.5 foi executado para:

1. transformar validações pontuais em rotinas operacionais contínuas (diário/semanal/mensal);
2. elevar robustez de recall explícito em janelas longas (T+1/T+7/T+30);
3. endurecer capacidade sob concorrência real (20/50/100 sessões);
4. reduzir ruído operacional em SLO (separando canário oficial de canário de depuração);
5. consolidar evidências auditáveis para decisão executiva de maturidade total.

### 9.2 Linha do tempo técnica executada

| Commit | Entrega | Resultado |
|---|---|---|
| `ffab794` | P2.1 carga e concorrência (script + evidência inicial) | Medição formal 20/50/100 criada |
| `c53a812` | P2.2 + P2.3 (auditoria semanal + governança mensal) | Rotinas e workflows implantados |
| `33b3a83` | Sincronização documental v9.2 | Matriz/checklist/report alinhados ao estado real |
| `efae08e` | Automação de streak 14 dias | Workflow diário consolidado + RPC diária |
| `598036d` | Correção de recall explícito | Priorização de `explicit_fact_store` no recall |
| `112aef9` | Hardening de capacidade para consulta composta | Carga 20/50/100 estabilizada com 100% |
| `57eb3ba` | Governança de SLO por canário oficial | `query_memory_slo` filtrando `slo_tracked=true` |
| `dc20414` | Evidência diária long horizon em Markdown | Relatório automático diário de T+1/T+7/T+30 |

### 9.3 Implementações detalhadas

#### 9.3.1 P2.2 Auditoria semanal de qualidade de memória

Implementado:

- script: `scripts/check_brain_memory_quality_audit.js`
- workflow: `.github/workflows/brain-memory-quality-audit-weekly.yml`
- comando: `npm run check:brain:memory-quality`
- evidência: `docs/brain_memory_quality_audit_20260303_170058.md`

Resultado semanal atual:

- amostras: `8`
- correta: `100%` (8/8)
- parcial/incorreta/desatualizada: `0%`

#### 9.3.2 P2.3 Governança mensal custo x qualidade

Implementado:

- script: `scripts/check_brain_cost_quality_governance.js`
- workflow: `.github/workflows/brain-cost-quality-governance-monthly.yml`
- comando: `npm run check:brain:governance-cost-quality`
- migration de suporte: `supabase/migrations/20260303143000_fix_query_telemetry_summary_service_role.sql`
- evidência: `docs/brain_memory_cost_quality_report_20260303_174352.md`

Resultado mensal (janela 30 dias):

- custo por interação: `$0.013119` (limite `$0.0100`)
- recall hit-rate: `90%` (meta `95%`)
- decisão: `RECUPERAR_QUALIDADE_E_CUSTO`

#### 9.3.3 Estabilidade diária (gate 14 dias)

Implementado:

- script de streak: `scripts/check_brain_memory_stability_streak.js`
- migration RPC diária: `supabase/migrations/20260303195000_add_query_memory_stability_daily_rpc.sql`
- workflow diário consolidado: `.github/workflows/brain-memory-long-horizon-daily.yml`
  - canário
  - suíte long horizon
  - SLO
  - streak
...
- `docs/brain_memory_stability_streak_report_20260303_205620.md`
- streak atual em 2026-03-03 UTC: `1/14`

#### 9.3.4 Correção de recall explícito (item 9)

Implementado no `chat-brain`:

- ordenação/deduplicação robusta de fatos explícitos recentes;
- prioridade explícita de `explicit_fact_store` antes de fallback cognitivo;
- estabilização da recuperação em janelas due.

Resultado validado:

- `T+1`: `PASS`, `source=explicit_fact_store`, marcador canônico LH;
- `T+7` e `T+30`: `PENDING` por janela temporal (ainda não due).

Evidência atual:

- `docs/brain_memory_long_horizon_report_20260303_205612.md`

#### 9.3.5 Hardening de capacidade (item 10)

Melhorias executadas no `chat-brain`:

- rota determinística para consultas compostas de contagem (`usuários + projetos`);
- fast-path de resposta SQL para contagem (redução de dependência de geração LLM);
- redução de custo/latência em caminho crítico de carga.

Comparativo de carga:

| Cenário | Relatório inicial | Relatório revalidado |
|---|---|---|
| 20 sessões | `PASS` 100% | `PASS` 100% |
| 50 sessões | `FAIL` 98% | `PASS` 100% |
| 100 sessões | `FAIL` 96% | `PASS` 100% |

Evidência final:

- `docs/brain_memory_load_report_20260303_204347.md`
- limite operacional recomendado: `100` sessões simultâneas

#### 9.3.6 SLO de memória com canário oficial (item 11)

Problema resolvido:

- canários manuais de depuração poluíam SLO de 24h.

Solução:

- `scripts/check_brain_canary.js` passou a registrar `params.slo_tracked`;
- workflow diário oficial injeta `BRAIN_CANARY_SLO_TRACKED=true`;
- `query_memory_slo` passou a considerar apenas runs com `slo_tracked=true`.

Migration:

- `supabase/migrations/20260303211000_filter_slo_to_tracked_canary_runs.sql`

Estado atual (2026-03-03 UTC):

- `overall=ok`
- `recall_hit_rate=100%`
- `critical_canary_failures=0`

### 9.4 Estado consolidado da matriz (itens 6–12)

| Item | Status em 2026-03-03 | Observação |
|---|---|---|
| 6 | Pronto | RPC determinística aplicada e validada |
| 7 | Pronto | Higiene de migrations regularizada |
| 8 | Pronto | Cron parametrizado e ativo por ambiente |
| 9 | Em risco | T+1 passou; T+7/T+30 aguardam janela due |
| 10 | Pronto | Carga 20/50/100 com 100% sucesso/consistência |
| 11 | Pronto | SLO 24h em `OK` com canário oficial tracked |
| 12 | Pronto | Runbook publicado e simulação executada |

### 9.5 Pendências remanescentes para "memória viva total"

Atualização em 3 de março de 2026 (pós-hotfix): a pendência de higiene operacional do canário foi resolvida com a RPC `cleanup_brain_canary_marker` e ajuste no script `check_brain_canary.js`; validação oficial executada com `5/5` testes passados e cleanup sem warning de schema.

1. concluir janelas `T+7` (9 de março de 2026) e `T+30` (1 de abril de 2026) sem regressão;
2. acumular `14` dias consecutivos estáveis no gate diário;
3. manter governança mensal de custo x qualidade até reduzir custo por interação ao limite alvo.

### 9.6 Sugestões objetivas de próximos passos (v9.5 → v9.6)

1. Em **9 de março de 2026 (T+7)**: executar e registrar rodada formal (`canário + long horizon + SLO + streak`) com fechamento documental.
2. Entre **10 e 31 de março de 2026**: manter rotina diária automática e revisar qualquer `ALERT` em até 15 minutos (SLO operacional).
3. Em **1 de abril de 2026 (T+30)**: validar `T+30=PASS`, atualizar matriz/checklist e recalcular gate final de maturidade.
4. Em **1 de abril de 2026** (governança mensal): reexecutar custo x qualidade e decidir entre `OTIMIZAR_CUSTO` ou `MANTER_E_MONITORAR`.

### 9.7 Encerramento da v9.5
