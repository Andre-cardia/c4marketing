-- Preserve historical MRR for cancelled/suspended/finalized contracts.
-- Contracts with a financial end date remain active up to expiration_date,
-- even if the current status is no longer active-like.
...
BEGIN
  ...
  normalized AS (
    SELECT
      cb.*,
      lower(trim(coalesce(cb.client_status, ''))) AS client_status_norm,
      coalesce(cb.billing_start_date, cb.accepted_at::date) AS effective_billing_start_date,
      (
  ...
  filtered AS (
    SELECT
      ...
      (
        (
          n.effective_billing_start_date IS NULL
          OR n.effective_billing_start_date <= v_reference_date
        )
        AND (
          (
            nullif(n.client_status_norm, '') IS NULL
            OR n.client_status_norm IN ('ativo', 'onboarding', 'em andamento')
          )
          OR (
            n.client_status_norm IN ('cancelado', 'suspenso', 'finalizado', 'inativo')
            AND n.expiration_date IS NOT NULL
            AND n.expiration_date >= v_reference_date
          )
        )
        AND (
          n.expiration_date IS NULL
          OR n.expiration_date >= v_reference_date
        )
      ) AS is_active_contract
    FROM normalized n
    WHERE (
      v_status_norm IS NULL
      OR v_status_norm = 'ativo'
      OR n.client_status_norm = v_status_norm
    )
  ),
  totals AS (
    SELECT
    ...
    'active_contracts', COALESCE(
      (
        SELECT json_agg(json_build_object(
          ...
          'company_alias', f.company_alias,
          'client_status', f.client_status,
          'accepted_at', f.accepted_at,
          'billing_start_date', f.billing_start_date,
          'effective_billing_start_date', f.effective_billing_start_date,
          'expiration_date', f.expiration_date,
          'monthly_fee', f.monthly_fee,
          'setup_fee', f.setup_fee,
          'has_traffic', f.has_traffic,
          'has_website', f.has_website,
          'has_landing_page', f.has_landing_page,
          'total_projects', f.total_projects
        ) ORDER BY f.company_name)
        FROM filtered f
        WHERE f.is_active_contract
      ),
      '[]'::json
    )
  )
