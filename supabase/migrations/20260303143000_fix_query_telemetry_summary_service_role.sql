-- Fix: allow service_role in telemetry summary role guard
-- Date: 2026-03-03
...
DECLARE
    ...
    v_jwt_role        TEXT;
BEGIN
    -- Acesso: gestor autenticado OU service_role (execucao operacional/CI)
    v_jwt_role := lower(coalesce(auth.jwt() ->> 'role', ''));
    SELECT role INTO v_user_role FROM public.app_users WHERE email = auth.jwt() ->> 'email';

    IF v_jwt_role <> 'service_role' AND v_user_role IS DISTINCT FROM 'gestor' THEN
        RAISE EXCEPTION 'Acesso negado: apenas gestores ou service_role podem acessar dados de telemetria.';
    END IF;
...
GRANT EXECUTE ON FUNCTION public.query_telemetry_summary(INT) TO authenticated, service_role;
