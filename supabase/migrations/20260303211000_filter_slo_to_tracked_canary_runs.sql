-- ============================================================
-- Scope memory SLO canary metrics to tracked runs only
-- ============================================================
...
BEGIN
  ...
  -- Only count canary runs explicitly tracked for SLO.
  SELECT
    count(*),
    count(*) FILTER (
  ...
  WHERE el.created_at >= v_cutoff
    ...
    AND coalesce((el.params->>'slo_tracked')::boolean, false) = true;
